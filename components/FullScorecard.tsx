...
const InningsCard: React.FC<{ innings: ScoreState, inningsNum: 1 | 2 }> = ({ innings, inningsNum }) => {
    const battingOrder = Object.values(innings.battingStats).sort((a,b) => (a.status === 'Not Out' ? -1 : 1));
            ...
            <StatTable title="Bowling" headers={['Bowler', 'Overs', 'Wickets',  'Wides']}>
                 {bowlingOrder.map(stats => (
                    <tr key={stats.playerId} className="border-b border-slate-700 hover:bg-slate-700/30">
                        ...
                        <td className="px-4 py-2">{stats.wickets}</td>
                        <td className="px-4 py-2">{stats.wides}</td>
                    </tr>
                ))}
            </StatTable>

            <div className="text-right text-slate-400 text-sm mt-4">
                Total: <span className="font-bold text-white">{innings.runs}/{innings.wickets}</span> in <span className="font-bold text-white">{`${innings.oversCompleted}.${innings.ballsInCurrentOver}`}</span> Overs
            </div>
        </div>
    );
}
